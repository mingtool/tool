...
class Smtp implements TransportInterface
{
    ...
    public function send(Message $message)
    {
        ...
        if ((count($recipients) == 0) && (!empty($headers) || !empty($body))) {
            ...
            sprintf(
                    '%s transport expects at least one recipient if the message has at least one header or body', __CLASS__
            )
            );
        }
    ...
    protected function prepareFromAddress(Message $message)
    {
        ...
        if (!count($from)) {
            ...
            throw new Exception\RuntimeException(sprintf(
                    '%s transport expects either a Sender or at least one From address in the Message; none provided', __CLASS__
            ));
        }
