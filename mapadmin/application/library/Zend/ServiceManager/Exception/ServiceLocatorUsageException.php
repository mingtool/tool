...
class ServiceLocatorUsageException extends ServiceNotFoundException
{
    ...
    AbstractPluginManager $pluginManager, ServiceLocatorInterface $parentLocator, $serviceName, BaseException $previousException
    )
    {
        return new self(
                sprintf(
                        ...
                        . 'did you forget to use $parentLocator = $serviceLocator->getServiceLocator() in your factory code?', $serviceName, get_class($pluginManager), get_class($previousException), $serviceName, get_class($parentLocator)
                ), 0, $previousException
        );
    }
