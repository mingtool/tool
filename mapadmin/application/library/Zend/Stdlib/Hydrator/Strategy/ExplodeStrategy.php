...
final class ExplodeStrategy implements StrategyInterface
{
    ...
    private function setValueDelimiter($delimiter)
    {
        if (!is_string($delimiter)) {
            throw new Exception\InvalidArgumentException(sprintf(
                    '%s expects Delimiter to be string, %s provided instead', __METHOD__, is_object($delimiter) ? get_class($delimiter) : gettype($delimiter)
            ));
        }
    ...
    public function hydrate($value)
    {
        ...
        if (!(is_string($value) || is_numeric($value))) {
            throw new Exception\InvalidArgumentException(sprintf(
                    '%s expects argument 1 to be string, %s provided instead', __METHOD__, is_object($value) ? get_class($value) : gettype($value)
            ));
        }
    ...
    public function extract($value)
    {
        if (!is_array($value)) {
            throw new Exception\InvalidArgumentException(sprintf(
                    '%s expects argument 1 to be array, %s provided instead', __METHOD__, is_object($value) ? get_class($value) : gettype($value)
            ));
        }
